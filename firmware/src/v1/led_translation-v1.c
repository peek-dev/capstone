...
uint8_t LEDTrans_Square(uint8_t row, uint8_t col) {
    ...
    // Skip the first row of pieces, a pawn if row<=6, and another if row == 1.
    uint8_t index = 8 + (row <= 5 ? 1 : 0) + (row == 0 ? 1 : 0);
    // Offset by the number of rows. The wiring is backwards, so higher row numbers
    // are actually lower indices in the LED chain.
    index += 8 * (7 - row);
    // Every other row goes backward, starting with row 8 (7).
